...
fun Route.flatRoute(){
    get("/flats"){
        call.respond(HttpStatusCode.OK, getFlats())
    }
                    ...
                    when (partData) {
                        is PartData.FormItem -> {
                            ...
                            when (partData.name) {
                                ...
                                "details" -> details = partData.value
    ...
    patch("/add-flat") {
        try {
            val newFlat=call.receive<Flat>()
            if (!addFlat(newFlat)){
                return@patch call.respond(HttpStatusCode.Conflict,SimpleResponse(successful = false, message = "Item already exits"))
            }
            call.respond(
                HttpStatusCode.Created, SimpleResponse(
                    successful = true,
                    message = "Successfully added ${newFlat.title}")
            )
        }catch (ex:Exception){
